...
2.  Generate a signed, short-lived handoff token on your server containing this distinct ID. Add this secure token (rather than the raw distinct ID) to the deep link as query parameters, along with other properties like UTM parameters.
3.  When the user is redirected to the app, parse the deep link, validate the handoff token through a backend API or server-side verification before calling any PostHog methods, and handle the following cases:

-   The mobile app is already authenticated. In this case, call [`posthog.alias()`](/docs/libraries/js/usage.md#alias) with the validated distinct ID. This associates the two distinct IDs as a single person.
-   The mobile app is unauthenticated. In this case, call [`posthog.identify()`](/docs/libraries/js/usage.md#identifying-users) with the validated distinct ID so pre-login mobile events stay connected to the web session. When the user later logs in on mobile, call `identify()` again with your canonical user ID.
...
class DeepLinkIdentityManager {
    ...
    func handleDeepLink(_ url: URL, isAuthenticatedOnMobile: Bool) {
        guard let handoffToken = URLComponents(url: url, resolvingAgainstBaseURL: true)?
            .queryItems?.first(where: { $0.name == "ph_handoff_token" })?.value else {
            return
        }
        // Validate the signed, short-lived handoff token (e.g., via backend API or local signature check)
        // to retrieve the validated distinct ID. Never pass raw query values directly.
        guard let webDistinctId = validateHandoffToken(handoffToken) else {
            return
        }
...
object DeepLinkIdentityManager {
    ...
    fun handleDeepLink(uri: Uri, isAuthenticatedOnMobile: Boolean) {
        val handoffToken = uri.getQueryParameter("ph_handoff_token") ?: return
        // Validate the signed, short-lived handoff token (e.g., via backend API or local signature check)
        // to retrieve the validated distinct ID. Never pass raw query values directly.
        val webDistinctId = validateHandoffToken(handoffToken) ?: return
        if (isAuthenticatedOnMobile) {
            // The mobile app already knows the current user.
            // Alias the incoming web distinct ID to that user.
